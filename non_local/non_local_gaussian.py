...
class NonLocalBlockND(object):
    ...
    def forward(self, x):
        ...
        theta_x = tf.reshape(x, (batch_size, -1, self.in_channels))

        if self.sub_sample:
            phi_x = tf.reshape(self.phi(x), (batch_size, self.in_channels, -1))
        else:
            phi_x = tf.reshape(x, (batch_size, self.in_channels, -1))

        f = tf.matmul(theta_x, phi_x)
        f_div_C = tf.nn.softmax(f)
